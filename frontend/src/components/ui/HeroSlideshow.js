...
const DEFAULT_SLIDES = [
  { img: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1600&q=85', label: 'Luxury Rooms & Suites' },
  { img: 'https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?w=1600&q=85', label: 'World-Class Suites' },
  { img: 'https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=1600&q=85', label: 'Rooftop Pool & Spa' },
  { img: 'https://images.unsplash.com/photo-1615460549969-36fa19521a4f?w=1600&q=85', label: 'Presidential Experience' },
  { img: 'https://images.unsplash.com/photo-1578683010236-d716f9a3f461?w=1600&q=85', label: 'Fine Dining & Bar' },
  { img: 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=1600&q=85', label: 'Elegant Interiors' },
];
...
export default function HeroSlideshow({ hotel, roomTypes = [] }) {
  ...
  return (
    <section className="relative min-h-[100svh] flex items-center overflow-hidden">
      {/* Background images — all rendered & crossfaded via opacity (no white flash) */}
      {slides.map((slide, i) => (
        <div
          key={i}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 pt-28 pb-24 sm:px-8 w-full">
        <div className="max-w-2xl text-white animate-fade-in-up">
          ...
          <div className="flex flex-wrap gap-4">
            <Link href="/rooms/book" className="btn-primary text-base">Explore &amp; Book</Link>
            <Link href="/about" className="btn-outline-light text-base">Discover More</Link>
          </div>
      ...
      {/* Navigation arrows */}
      <button onClick={prev} aria-label="Previous slide" className="absolute left-4 top-1/2 -translate-y-1/2 z-20 bg-black/30 hover:bg-black/60 text-white rounded-full w-11 h-11 flex items-center justify-center backdrop-blur-sm transition-all hover:scale-110"><ChevronLeft className="w-5 h-5" /></button>
      <button onClick={next} aria-label="Next slide" className="absolute right-4 top-1/2 -translate-y-1/2 z-20 bg-black/30 hover:bg-black/60 text-white rounded-full w-11 h-11 flex items-center justify-center backdrop-blur-sm transition-all hover:scale-110"><ChevronRight className="w-5 h-5" /></button>

      {/* Dot indicators */}
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2 items-center">
        {slides.map((_, i) => (
          <button key={i} onClick={() => go(i)} aria-label={`Go to slide ${i + 1}`} className={`rounded-full transition-all duration-300 ${i === current ? 'w-8 h-2 bg-white' : 'w-2 h-2 bg-white/50 hover:bg-white/80'}`} />
        ))}
      </div>
